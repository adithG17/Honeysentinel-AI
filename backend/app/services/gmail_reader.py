...
def get_gmail_body(payload):
    ...
    def is_placeholder(content):
        return ("gmail client cannot display HTML" in content.lower() or 
...
async def get_gmail_authenticity(gmail_address):
    ...
    # Improved gmail pattern that handles various formats:
...
def extract_links(html_content):
    ...
    # Simple regex to find <a href=""> tags
        ...
        try:
            ...
            if parsed.scheme in ('http', 'https', 'mailto'):
                parsed_links.append({
                    ...
                    'is_external': bool(parsed.netloc)  # True if not empty
                })
        except:
            continue
...
async def fetch_gmail_messages(max_results=10, include_authenticity=False):
    ...
    for msg in messages:
        ...
        if include_authenticity:
            ...
            gmail_msg = message_from_bytes(raw_data)
        else:
            gmail_msg = None
        ...
        links = []
        if body_data["html"]:
            links = extract_links(body_data["html"])
        ...
        if include_authenticity and from_addr:
            authenticity = await get_gmail_authenticity(from_addr)
